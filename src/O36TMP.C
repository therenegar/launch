...
static int row_index(int c,int row){int ff=first_face(c),st=sol_vstart(c),en=sol_vend(c),n=en-st,off=5+ff;if(ff>0&&row>=5&&row<5+ff)return-4;if(tn[c]-ff>sol_cap(c)&&row==4)return-2;if(row==off+n)return-3;if(row<off||row>=off+n)return-1;return st+(row-off);}
...
for(c=0;c<COLS;c++){for(i=5;i<=16;i++){acc_put(x+2+c*10,y+i,178,ACC_ATTR(SOL_BOARD_BG,SOL_BOARD_FG));acc_put(x+8+c*10,y+i,178,ACC_ATTR(SOL_BOARD_BG,SOL_BOARD_FG));}}if(erase){acc_fill(x+1,y+1,71,19,' ',ACC_BG);acc_fill(x+1,y+4,71,14,178,ACC_ATTR(SOL_BOARD_BG,SOL_BOARD_FG));acc_fill(x+1,y+18,71,1,' ',ACC_BG);}if(ns)draw_back(x+3,y+2,0,0);else draw_empty(x+3,y+2,0);if(kbd_nav&&focus==0)card_selected(x+3,y+2,0,0);if(nw){wshow=nw<3?nw:3;for(i=wshow-1;i>0;i--)draw_waste_under(x+19-i*4,y+2,waste[nw-1-i]);draw_face(x+19,y+2,waste[nw-1],0,0);if(kbd_nav&&focus==1&&!selected)card_selected(x+19,y+2,0,0);if(selected==1){card_selected(x+19,y+2,0,0);if(kbd_dest)card_shadow(x+19,y+2,SOL_BOARD_BG);}}else{draw_empty(x+19,y+2,0);if(kbd_nav&&focus==1)card_selected(x+19,y+2,0,0);}for(i=0;i<4;i++){if(foundation[i]){draw_rank(x+38+i*8,y+2,foundation[i],i,0);if(kbd_nav&&focus==2+i&&!selected)card_selected(x+38+i*8,y+2,0,0);if(selected==3&&selcol==i){card_selected(x+38+i*8,y+2,0,0);if(kbd_dest)card_shadow(x+38+i*8,y+2,SOL_BOARD_BG);}}else{q[0]='|';q[1]=' ';q[2]=(char)suit_glyph[i];q[3]=' ';q[4]='|';q[5]=0;acc_text(x+38+i*8,y+2,q,ACC_BORDER,5);if(kbd_nav&&focus==2+i&&!selected)card_selected(x+38+i*8,y+2,0,0);}}for(c=0;c<COLS;c++){acc_fill(x+2+c*10,y+5,1,12,178,ACC_ATTR(SOL_BOARD_BG,SOL_BOARD_FG));acc_fill(x+8+c*10,y+5,1,12,178,ACC_ATTR(SOL_BOARD_BG,SOL_BOARD_FG));st=sol_vstart(c);en=sol_vend(c);row=5;if(!tn[c]){draw_empty(x+3+c*10,y+5,0);if(kbd_nav&&!selected&&focus==6+c)card_selected(x+3+c*10,y+5,0,0);}for(i=0;i<first_face(c)&&row<=16;i++,row++)draw_back(x+3+c*10,y+row,0,i<first_face(c)-1);if(tn[c]-first_face(c)>sol_cap(c))hidden_mark(x+3+c*10,y+4);for(i=st;i<en&&row<=16;i++,row++){id=tab[c][i];draw_face(x+3+c*10,y+row,id,0,i<tn[c]-1);}if(selected==2&&selcol==c&&selidx>=st&&selidx<en){sy=5+first_face(c)+(selidx-st);if(sy<=16){card_selected(x+3+c*10,y+sy,0,0);if(kbd_dest)card_shadow(x+3+c*10,y+sy,SOL_BOARD_BG);}}else if(kbd_nav&&!selected&&focus==6+c&&cursor[c]>=st&&cursor[c]<en){sy=5+first_face(c)+(cursor[c]-st);if(sy<=16)card_selected(x+3+c*10,y+sy,0,0);}if(selected&&valid_tableau_dest(c)){int dr=(tn[c]-first_face(c)>sol_cap(c))?16:5+first_face(c)+(tn[c]-first_face(c));if(dr>16)dr=16;destination_mark(x+3+c*10,y+dr,focus==6+c);}}if(selected)for(i=0;i<4;i++)if(valid_foundation_dest(i))destination_mark(x+38+i*8,y+2,focus==2+i);game_draw_status();}
...
static void dest_pos(int f,int *px,int *py){if(f==1){*px=19;*py=2;}else if(f>=2&&f<=5){*px=38+(f-2)*8;*py=2;}else{int c=f-6;*px=3+c*10;*py=(tn[c]-first_face(c)>sol_cap(c))?16:5+first_face(c)+(tn[c]-first_face(c));}}
