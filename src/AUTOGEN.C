/* AutoGen 3.11 - automatic Launch! menu generator for DOS.
...
/* Print normally first, then recolour only the cells already written. */
static void colour_text(const char *s,int attr)
{
  union REGS r;
  unsigned char mode,page;
  unsigned cols,start_row,start_col,row,col;
  unsigned short far *video;
  unsigned seg;
  const char *p;
  if(!_isatty(_fileno(stdout))){fputs(s,stdout);return;}
  fflush(stdout);memset(&r,0,sizeof(r));r.h.ah=0x0F;int86(0x10,&r,&r);
  mode=r.h.al;cols=r.h.ah?r.h.ah:80;page=r.h.bh;
  r.h.ah=3;r.h.bh=page;int86(0x10,&r,&r);start_row=r.h.dh;start_col=r.h.dl;
  fputs(s,stdout);fflush(stdout);
  seg=(mode==7)?0xB000:0xB800;video=(unsigned short far *)((unsigned long)seg<<16);
  row=start_row;col=start_col;
  for(p=s;*p;p++){
    if(*p=='\r'){col=0;continue;}
    if(*p=='\n'){row++;continue;}
    video[row*cols+col]=(video[row*cols+col]&0x00FF)|((unsigned short)(unsigned char)attr<<8);
    if(++col>=cols){col=0;row++;}
  }
}
static void status_icon(int colour,int symbol)
{
  char block[2],mark[2];
  block[0]=(char)219;block[1]=0;mark[0]=(char)symbol;mark[1]=0;
  colour_text(block,colour);colour_text(mark,(colour<<4)|15);colour_text(block,colour);putchar(' ');
}
static void question_icon(void){status_icon(1,'?');}
static void error_icon(void){status_icon(4,'!');}
static void default_choice(const char *value){putchar('[');colour_text(value,10);fputs("]: ",stdout);}
...
static int choose_match(const char *path,int count)
{
  ...
  printf("  0. Skip\n");question_icon();printf("Choose ");default_choice("0");
...
static int write_menu(void)
{
  ...
  fputs("; Launch! 3.11 menu generated by AutoGen\n\n[Launcher]\n",f);
...
int main(int argc,char **argv)
{
  ...
  puts("AutoGen 3.11 - Launch! Program Scanner\n");
  ...
  if(!database_is_valid()){error_icon();printf("Cannot read a valid program database: %s\n",database_file);return 1;}
  if(exists(menu_file)){
    question_icon();printf("LAUNCH.MNU already exists. Replace it with a generated menu? [y/");colour_text("N",10);fputs("]: ",stdout);
  ...
  add_dos_utilities();
  if(!scan_drives(drives)){error_icon();puts("Unable to create the scan work file.");return 1;}
  if(!write_menu()){error_icon();puts("Could not safely write LAUNCH.MNU.");return 1;}
  ...
  if(found_count==MAX_FOUND){error_icon();puts("The 70-item Launch! tree limit was reached; additional matches were omitted.");}
