/* AutoGen 1.5 - automatic Launch! menu generator for DOS.
...
static void add_dos_utilities(void)
{
  ...
  add_dos_item("Disk Partition",category,"FDISK",1);
  add_dos_item("System Transfer",category,"SYS",0);
  if(major>=4||is_free)add_dos_item("Memory Status",category,"MEM",1);
  if(major>=5||is_free){
    add_dos_item(is_pc?"E Editor":"DOS Editor",category,is_pc?"E":"EDIT",1);
    add_dos_item("DOS Help",category,"HELP",1);
  }
  if(major>=6||is_free){
  ...
  add_available_dos_item("Sort Text File",category,"SORT",0);
}
...
static void write_category(FILE *f,const char *cat,int start,int count)
{
  ...
  while(remain>0&&level<3){
    ...
    for(i=0;i<take;i++)fprintf(f,"ITEM=%s|%s|%d|%d\n",found[pos+i].title,found[pos+i].command,
  ...
  if(remain>0){
    fprintf(f,"\n[%s]\n",section);
    for(i=0;i<remain&&i<20;i++)fprintf(f,"ITEM=%s|%s|%d|%d\n",found[pos+i].title,found[pos+i].command,
      found[pos+i].press_enter,found[pos+i].change_directory);
  }
}
...
static int write_menu(void)
{
  ...
  fputs("; Launch! 1.5 menu generated by AutoGen\n\n[Launcher]\n",f);
...
int main(int argc,char **argv)
{
  ...
  puts("AutoGen 1.5 - Launch! Program Scanner\n");
  ...
  puts("This may take several minutes. Press Ctrl+C to cancel.\n");
  add_dos_utilities();
  ...
  printf("DOS utilities added: %d. Recognised applications added: %d.\n",
