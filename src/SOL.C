...
static void card_shadow(int x,int y,int background){int i;acc_put(x+5,y,220,ACC_ATTR(background,0));for(i=1;i<=5;i++)acc_put(x+i,y+1,223,ACC_ATTR(background,0));}
...
static void render(int x,int y,int focus,int erase){char q[6];int c,i,ff,row,id,mark,top,wshow,sy;if(erase){acc_fill(x+1,y+1,71,19,' ',ACC_BG);acc_fill(x+1,y+4,71,14,178,ACC_ATTR(SOL_BOARD_BG,SOL_BOARD_FG));acc_fill(x+1,y+18,71,1,' ',ACC_BG);}if(ns)draw_back(x+3,y+2,focus==0,0);else draw_empty(x+3,y+2,focus==0);if(nw){wshow=nw<3?nw:3;for(i=wshow-1;i>0;i--)draw_waste_under(x+19-i*4,y+2,waste[nw-1-i]);draw_face(x+19,y+2,waste[nw-1],focus==1&&!selected,0);if(selected==1)card_shadow(x+19,y+2,acc_appearance.background);}else draw_empty(x+19,y+2,focus==1);for(i=0;i<4;i++){if(foundation[i]){draw_rank(x+38+i*8,y+2,foundation[i],i,focus==2+i&&!selected);if(selected==3&&selcol==i)card_shadow(x+38+i*8,y+2,acc_appearance.background);}else{q[0]='|';q[1]=' ';q[2]=(char)suit_glyph[i];q[3]=' ';q[4]='|';q[5]=0;acc_text(x+38+i*8,y+2,q,focus==2+i?ACC_SELECT:ACC_BORDER,5);}}for(c=0;c<COLS;c++){ff=first_face(c);row=4;top=tn[c]-1;if(ff>0){draw_back(x+3+c*10,y+row,focus==6+c&&cursor[c]<ff&&!selected,1);row++;}for(i=ff;i<tn[c]&&row<=17;i++,row++){id=tab[c][i];mark=(focus==6+c&&cursor[c]==i&&!selected);draw_face(x+3+c*10,y+row,id,mark,i<top);}if(selected==2&&selcol==c&&selidx>=ff&&selidx<tn[c]){sy=4+(ff>0)+(selidx-ff);if(sy<=17)card_shadow(x+3+c*10,y+sy,SOL_BOARD_BG);}if(!tn[c])draw_empty(x+3+c*10,y+4,focus==6+c&&!selected);}acc_text(x+18,y+19,"Select a card, then its destination",ACC_LABEL,36);}
