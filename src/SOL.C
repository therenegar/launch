...
static int move_foundation(int suit){int id=source_id();if(id<0||(int)card[id].suit!=suit||(int)card[id].rank!=(int)foundation[suit]+1)return 0;if(selected==2&&selidx!=tn[selcol]-1)return 0;if(selected==3)return 0;foundation[suit]++;remove_source();return 1;}
...
{int a=focused?ACC_SELECT:ACC_ATTR(2,15);acc_text(x,y,"|   |",a,5);}
