...
int main(int argc,char**argv){union REGS q;int m,yr,tm,ty,td,key=0,x,y,mx=0,my=0,focus=-1,dirty=1,i,d,st;unsigned b=0;char path[ACC_PATH],*file=0;if(acc_help(argc,argv,"!CAL","Displays, prints and views iCalendar events."))return 0;if(!acc_begin(argv[0],"Calendar",0))return 1;for(i=1;i<argc;i++)if(!strnicmp(argv[i],"/FILE=",6))file=argv[i]+6;if(file){if(strchr(file,'\\')||strchr(file,':'))strcpy(path,file);else acc_path(path,"",file);}else acc_path(path,"","CAL.ICS");load_ics(path);q.h.ah=0x2A;int86(0x21,&q,&q);tm=q.h.dh-1;ty=q.x.cx;td=q.h.dl;m=tm;yr=ty;x=(acc_cols-61)/2;y=(acc_rows-21)/2;acc_box(x,y,61,21,"Calendar");while(key!=27){if(dirty){draw_month(x,y,m,yr,tm,ty,td);dirty=0;}acc_button(x+3,y+18,"  \021  ",focus==0);acc_button(x+10,y+18,"  Today  ",focus==1);acc_button(x+21,y+18,"  \020  ",focus==2);acc_button(x+28,y+18,"  Print  ",focus==3);acc_button(x+51,y+18,"  Close  ",focus==4);acc_wait(&key,&mx,&my,&b);if((b&1)&&my>=y+5&&my<y+17&&mx>=x+2&&mx<x+59){int cc=(mx-(x+2))/8,rr=(my-(y+4))/2;st=weekday(yr,m,1);d=rr*7+cc-st+1;if(d>=1&&d<=mdays(m,yr)&&has_event(yr,m,d)){day_dialog(yr,m,d);acc_box(x,y,61,21,"Calendar");dirty=1;key=0;continue;}}if((b&1)&&my==y+18){if(mx>=x+2&&mx<x+8)focus=0;else if(mx>=x+9&&mx<x+19)focus=1;else if(mx>=x+20&&mx<x+26)focus=2;else if(mx>=x+27&&mx<x+37)focus=3;else if(mx>=x+51&&mx<x+59)focus=4;key=13;}if(key==9||key==271){if(focus<0)focus=(key==271)?4:0;else focus=(key==271)?(focus+4)%5:(focus+1)%5;key=0;}if(key==256+71){m=tm;yr=ty;dirty=1;key=0;}else if(key==256+75){if(--m<0){m=11;yr--;}dirty=1;key=0;}else if(key==256+77){if(++m>11){m=0;yr++;}dirty=1;key=0;}else if(key==13){if(focus==0){if(--m<0){m=11;yr--;}dirty=1;}else if(focus==1){m=tm;yr=ty;dirty=1;}else if(focus==2){if(++m>11){m=0;yr++;}dirty=1;}else if(focus==3){if(!print_month(m,yr))acc_notice("Print","Unable to print calendar to LPT1.");}else key=27;if(key!=27)key=0;}else if(key!=27)key=0;}acc_end();return 0;}
