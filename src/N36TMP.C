...
int main(int argc,char**argv)
{
 ...
 while(key!=27){int last;
  ...
  if(mb&1){
   if(my==y+2){last=tab_last_visible(tabfirst);if(mx==x+3&&tabfirst>0){tabfirst--;draw_tabs(x,y+2,tabfirst,focus==0);key=0;continue;}if(mx==x+65&&last+1<ntab){tabfirst++;draw_tabs(x,y+2,tabfirst,focus==0);key=0;continue;}{int i,xx=x+6;for(i=tabfirst;i<=last&&i<ntab;i++){int w=(int)strlen(tabs[i])+4;if(mx>=xx&&mx<xx+w){page_save();ctab=i;cpage=psel=0;pagefirst=0;page_load();cx=cy=top=0;focus=0;note_editor_focus=0;row_draw(tx,ty,cy,top);draw_tabs(x,y+2,tabfirst,1);view_draw(tx,ty,cx,cy,top);draw_pages(x+65,ty,pagefirst,0,psel);key=0;break;}xx+=w+1;}if(i<=last)continue;}}
   ...
   if(my>=ty&&my<ty+NV&&mx>=tx+4&&mx<tx+4+EW){int ocx=cx,ocy=cy;focus=1;note_editor_focus=1;cx=mx-(tx+4);cy=top+my-ty;row_draw(tx,ty,ocy,top);if(cy!=ocy)row_draw(tx,ty,cy,top);cursor_draw(tx,ty,cx,cy,top);key=0;continue;}
   if(mx>=x+65&&mx<x+68&&my>=ty&&my<ty+NV){int rel=my-ty,lastp=page_last_visible(pagefirst);focus=2;note_editor_focus=0;row_draw(tx,ty,cy,top);if(rel==0&&pagefirst>0){pagefirst--;draw_pages(x+65,ty,pagefirst,1,psel);key=0;continue;}if(rel==11&&lastp+1<(int)pages[ctab]){pagefirst++;draw_pages(x+65,ty,pagefirst,1,psel);key=0;continue;}if(rel>=1&&rel<=10){int pg=pagefirst+rel-1;if(pg<(int)pages[ctab]){page_save();cpage=psel=pg;page_load();cx=cy=top=0;view_draw(tx,ty,cx,cy,top);draw_pages(x+65,ty,pagefirst,1,psel);key=0;continue;}if(pg==(int)pages[ctab]&&(int)pages[ctab]<MAXPAGES){page_save();pages[ctab]++;cpage=psel=pages[ctab]-1;memset(note,' ',sizeof(note));memset(softwrap,0,sizeof(softwrap));meta_save();page_save();cx=cy=top=0;view_draw(tx,ty,cx,cy,top);draw_pages(x+65,ty,pagefirst,1,psel);key=0;continue;}}}
  ...
  if(key==9||key==271){focus=note_next_focus(focus,key==271);note_editor_focus=(focus==1);row_draw(tx,ty,cy,top);cursor_draw(tx,ty,cx,cy,top);key=0;draw_tabs(x,y+2,tabfirst,focus==0);draw_pages(x+65,ty,pagefirst,focus==2,psel);continue;}
  if(focus==0&&key==13){strcpy(nm,tabs[ctab]);if(input_name(nm,1)){strcpy(tabs[ctab],nm);meta_save();}acc_box(x,y,70,20,"Note");draw_tabs(x,y+2,tabfirst,1);view_draw(tx,ty,cx,cy,top);draw_pages(x+65,ty,pagefirst,0,psel);key=0;continue;}
  if(focus==0&&(key==256+75||key==256+77||key==' ')){page_save();if(key==256+75&&ctab>0)ctab--;else if(key==256+77&&ctab+1<ntab)ctab++;if(ctab<tabfirst)tabfirst=ctab;last=tab_last_visible(tabfirst);while(ctab>last){tabfirst++;last=tab_last_visible(tabfirst);}cpage=psel=pagefirst=0;page_load();cx=cy=top=0;view_draw(tx,ty,cx,cy,top);draw_tabs(x,y+2,tabfirst,1);draw_pages(x+65,ty,pagefirst,0,psel);key=0;continue;}
