 ...
 * File: NOTEBLD.C
 * Role: Build copy of !NOTE
 * Build/ownership: Derived from NOTE.C.
 * Maintainer contract: Keep synchronized with NOTE.C.
...
#define NOTE_CLIP_MAX (MAX_EW*NL+NL*2+1)
...
static void note_resolve_path(const char *name,char *full){if(strchr(name,'\\')||strchr(name,'/')||strchr(name,':')){strncpy(full,name,ACC_PATH-1);full[ACC_PATH-1]=0;}else sprintf(full,"%s\\%s",acc_directory,name);}
static int note_external_prepare(void){int found=-1,i;if(external_tab>=0)return 1;for(i=0;i<persistent_ntab;i++)if(!stricmp(tabs[i],"External")){found=i;break;}if(found>=0){external_tab=found;external_saved_pages=pages[found];external_runtime_tab=0;}else if(ntab<MAXTABS){external_tab=ntab;external_saved_pages=0;external_runtime_tab=1;strcpy(tabs[ntab],"External");pages[ntab]=0;ntab++;}else return 0;return 1;}
static int note_ascii_file(const char *path){FILE *f;int c;f=fopen(path,"rb");if(!f)return 0;while((c=fgetc(f))!=EOF){if(c>127||c==0||(c<32&&c!='\r'&&c!='\n'&&c!='\t')){fclose(f);return 0;}}fclose(f);return 1;}
static int note_open_files(const char *list){char work[ACC_PATH],*q,*start,full[ACC_PATH],msg[ACC_PATH+32];int added=0,n;strncpy(work,list,ACC_PATH-1);work[ACC_PATH-1]=0;q=work;while(*q){while(*q==' ')q++;if(!*q)break;start=q;while(*q&&*q!=' ')q++;if(*q)*q++=0;note_resolve_path(start,full);if(!acc_exists(full)){sprintf(msg,"File %s doesn't exist!",start);acc_notice("Error",msg);continue;}if(!note_ascii_file(full)){acc_notice("Error","Unsupported file type! ASCII text files only.");continue;}if(external_count>=MAX_EXTERNAL){acc_notice("Open","No more external pages are available.");break;}if(!note_external_prepare()){acc_notice("Open","No tab is available for external files.");break;}n=external_count++;strncpy(external_path[n],full,ACC_PATH-1);external_path[n][ACC_PATH-1]=0;external_temp_name(external_tmp[n],n);remove(external_tmp[n]);external_dirty[n]=0;added++;}if(added){pages[external_tab]=(unsigned char)external_count;page_save();ctab=external_tab;cpage=0;return 1;}return 0;}
...
{
 if(ctab==external_tab&&external_count>0)return 1;
...
static int export_current_page(char *out)
{
 ...
 last=NL-1;while(last>=0){q=EW;while(q&&note[last*NW+q-1]==' ')q--;if(q)break;last--;}
 for(r=0;r<=last;r++){q=EW;while(q&&note[r*NW+q-1]==' ')q--;fwrite(note+r*NW,1,q,f);fputc('\n',f);}
 fclose(f);return 1;
}
...
static void draw_tabs(int x,int y,int first,int focus)
{
 ...
 for(i=first;i<=last&&i<ntab;i++){int a=(i==ctab)?ACC_CONTROL:ACC_SELECT,w=(int)strlen(tabs[i])+4;acc_fill(xx,y,w,1,' ',a);acc_text(xx+2,y,tabs[i],a,(int)strlen(tabs[i]));if((focus&&i==ctab)||i==note_tab_hover){int fa=ACC_ATTR(acc_appearance.controls_bg,acc_appearance.controls_fg);acc_put(xx,y,169,fa);acc_put(xx+w-1,y,170,fa);}xx+=w+1;}if(last==ntab-1&&ntab<MAXTABS&&xx<x+65)acc_put(xx,y,'+',ACC_ATTR(acc_appearance.background,acc_appearance.titlebar_fg));
}
...
{
 int i,xx=x,last=page_last_visible(first),a,w,n,max,ep,right=(note_edit_width==MAX_EW)?80:x+EW+3,fillw=(note_edit_width==MAX_EW)?80-x:EW+1;char b[6],shown[30],*base;(void)first;
 acc_fill(x,y,fillw,1,' ',ACC_BG);
 for(i=0;i<=last;i++){
...
{
 int cy=*pcy,w=note_edit_width,start=w-1,len,i,can=1;if(cy>=NL-1)return;
 if(note[cy*NW+w-1]==' '){*pcy=cy+1;*pcx=0;softwrap[cy+1]=1;return;}
 while(start>0&&note[cy*NW+start-1]!=' ')start--;
 if(start<=0){*pcy=cy+1;*pcx=0;softwrap[cy+1]=1;return;}
 len=w-start;for(i=w-len;i<w;i++)if(note[(cy+1)*NW+i]!=' ')can=0;
 if(!can){*pcy=cy+1;*pcx=0;softwrap[cy+1]=1;return;}
 memmove(note+(cy+1)*NW+len,note+(cy+1)*NW,w-len);memcpy(note+(cy+1)*NW,note+cy*NW+start,len);memset(note+cy*NW+start,' ',w-start);softwrap[cy+1]=1;*pcy=cy+1;*pcx=len;
}
...
{
 int lo,hi,p,row,col,lastrow,n=0,w=note_edit_width;if(!note_has_selection())return;lo=note_sel_low();hi=note_sel_high();lastrow=(hi-1)/w;
 for(p=lo;p<hi&&n<NOTE_CLIP_MAX-3;){
  row=p/w;col=p%w;note_clip[n++]=note[row*NW+col];p++;
  if(p<hi&&p%w==0&&row<lastrow&&!softwrap[row+1]){note_clip[n++]='\r';note_clip[n++]='\n';}
 }
 note_clip_len=n;
}
static void note_delete_selection(int *pcx,int *pcy)
{
 int lo,hi,fr,lr,r,start,end,count,w=note_edit_width;if(!note_has_selection())return;
 lo=note_sel_low();hi=note_sel_high();fr=lo/w;lr=(hi-1)/w;
 for(r=fr;r<=lr;r++){start=(r==fr)?lo%w:0;end=(r==lr)?((hi-1)%w)+1:w;count=end-start;if(count>0){memmove(note+r*NW+start,note+r*NW+end,w-end);memset(note+r*NW+w-count,' ',count);}}
 *pcy=fr;*pcx=lo%w;note_selection_clear();
}
static void note_insert_one(int *pcx,int *pcy,int ch,int insert)
{
 int cx=*pcx,cy=*pcy,base=cy*NW,w=note_edit_width;if(insert&&cx<w-1)memmove(note+base+cx+1,note+base+cx,w-cx-1);note[base+cx]=(char)ch;if(cx<w-1)cx++;else if(cy<NL-1)note_word_wrap(&cx,&cy);*pcx=cx;*pcy=cy;
}
...
static int note_used(int row){int n=note_edit_width;while(n>0&&note[row*NW+n-1]==' ')n--;return n;}
static void note_remove_row(int row){int r,w=note_edit_width;for(r=row;r<NL-1;r++){memcpy(note+r*NW,note+(r+1)*NW,w);memset(note+r*NW+w,' ',NW-w);softwrap[r]=softwrap[r+1];}memset(note+(NL-1)*NW,' ',NW);softwrap[NL-1]=0;}
static void note_join_next(int *pcx,int *pcy){int cx=*pcx,cy=*pcy,n,take,room,w=note_edit_width;if(cy>=NL-1)return;n=note_used(cy+1);room=w-cx;take=n<room?n:room;if(take)memcpy(note+cy*NW+cx,note+(cy+1)*NW,take);if(take>=n)note_remove_row(cy+1);else{memmove(note+(cy+1)*NW,note+(cy+1)*NW+take,w-take);memset(note+(cy+1)*NW+w-take,' ',take);}}
static void note_join_previous(int *pcx,int *pcy){int cx,cy=*pcy,prev,n,take,room,w=note_edit_width;if(cy<=0)return;prev=note_used(cy-1);n=note_used(cy);room=w-prev;if(room<=0){*pcy=cy-1;*pcx=w-1;return;}take=n<room?n:room;if(take)memcpy(note+(cy-1)*NW+prev,note+cy*NW,take);if(take>=n)note_remove_row(cy);else{memmove(note+cy*NW,note+cy*NW+take,w-take);memset(note+cy*NW+w-take,' ',take);}cx=prev;*pcy=cy-1;*pcx=cx;}
static void note_split_line(int *pcx,int *pcy){int cx=*pcx,cy=*pcy,r,n,tail,indent=0,w=note_edit_width;if(cy>=NL-1)return;while(indent<w&&note[cy*NW+indent]==' ')indent++;if(indent>=w)indent=0;for(r=NL-1;r>cy+1;r--){memcpy(note+r*NW,note+(r-1)*NW,w);memset(note+r*NW+w,' ',NW-w);softwrap[r]=softwrap[r-1];}memset(note+(cy+1)*NW,' ',NW);n=note_used(cy);tail=n>cx?n-cx:0;if(tail>w-indent)tail=w-indent;if(tail)memcpy(note+(cy+1)*NW+indent,note+cy*NW+cx,tail);memset(note+cy*NW+cx,' ',w-cx);softwrap[cy+1]=0;*pcy=cy+1;*pcx=indent;}
...
{
 int w=50,h=9,x=(acc_cols-w)/2,y=(acc_rows-h)/2,k=0,mx=0,my=0,pos=(int)strlen(out),f=0;unsigned mb=0;
 acc_modal_begin();for(;;){acc_subbox(x,y,w,h,"Save Note",1);acc_text(x+3,y+2,"Filename:",ACC_LABEL,10);acc_fill(x+13,y+2,32,1,' ',f==0?ACC_SELECT:ACC_CONTROL);acc_text(x+13,y+2,out,f==0?ACC_SELECT:ACC_CONTROL,31);if(f==0)acc_caret_set(x+13+(pos<31?pos:30),y+2);else acc_caret_hide();acc_button(x+3,y+6," Save ",f==1);acc_button(x+11,y+6," Cancel ",f==2);acc_wait(&k,&mx,&my,&mb);if(k==27){acc_modal_end();return 0;}if((mb&1)&&my==y+2){f=0;k=0;}else if((mb&1)&&my==y+6&&mx>=x+3&&mx<x+9){f=1;k=13;}else if((mb&1)&&my==y+6&&mx>=x+11&&mx<x+19){f=2;k=13;}if(k==9||k==271){f=k==271?(f+2)%3:(f+1)%3;k=0;continue;}if(k==13&&f==2){acc_modal_end();return 0;}if(k==13&&(f==0||f==1)){if(pos){acc_modal_end();return 1;}k=0;continue;}if(f==0){int n=(int)strlen(out);if(k==256+71)pos=0;else if(k==256+79)pos=n;else if(k==8&&pos){memmove(out+pos-1,out+pos,n-pos+1);pos--;}else if(k>=32&&k<127&&n<ACC_PATH-1){memmove(out+pos+1,out+pos,n-pos+1);out[pos++]=(char)k;}}k=0;}
}
static int note_write_current_path(const char *path)
{
 FILE *f;int r,q,last=NL-1;while(last>=0){q=note_edit_width;while(q&&note[last*NW+q-1]==' ')q--;if(q)break;last--;}f=fopen(path,"wb");if(!f)return 0;for(r=0;r<=last;r++){q=note_edit_width;while(q&&note[r*NW+q-1]==' ')q--;if(q)fwrite(note+r*NW,1,q,f);if(r<last&&!softwrap[r+1])fputs("\r\n",f);}fclose(f);return 1;
}
static int note_save_current_as(void)
{
  char name[ACC_PATH],full[ACC_PATH],msg[ACC_PATH+24];
  int ep=external_page_index();

  /* Save means overwrite for a page that came from an existing file.
     Only an unnamed/new page asks the user for a filename. */
  if(ep>=0){
    if(!note_write_current_path(external_path[ep])){
      acc_notice("Save Error","Unable to save opened file.");
      return 0;
    }
    external_work_save(ep);
    external_dirty[ep]=0;
...
{
 char far *tmp;unsigned char ns[NL];char pending[MAX_EW*2+2];int old=note_edit_width,last,y,e,r,n,nextn,i,k,plen,cut,drop,out=0,cont,emitted;int target_para=0,start=0,target_row=0,hard=0,sep;long off=0,remain;
 if(new_width==old)return 1;if(new_width<EW)new_width=EW;if(new_width>MAX_EW)new_width=MAX_EW;
 start=*pcy;while(start>0&&softwrap[start])start--;for(r=1;r<=start;r++)if(!softwrap[r])target_para++;for(r=start;r<*pcy;r++){n=note_used(r);off+=n;if(r+1<NL&&softwrap[r+1]&&n&&note_used(r+1))off++;}n=note_used(*pcy);off+=*pcx<n?*pcx:n;
 tmp=(char far *)_fmalloc((unsigned)(NW*NL));if(!tmp)return 0;_fmemset(tmp,' ',(unsigned)(NW*NL));memset(ns,0,sizeof(ns));
 last=NL-1;while(last>0&&note_used(last)==0&&!softwrap[last])last--;y=0;
 while(y<=last&&out<NL){e=y;while(e+1<=last&&softwrap[e+1])e++;plen=0;cont=0;emitted=0;
  for(r=y;r<=e;r++){n=note_used(r);for(i=0;i<n;i++){pending[plen++]=note[r*NW+i];if(plen>new_width){cut=0;for(k=new_width;k>0;k--)if(pending[k]==' '){cut=k;break;}drop=cut?1:0;if(!cut)cut=new_width;if(out>=NL)break;_fmemcpy(tmp+out*NW,pending,cut);ns[out]=(unsigned char)(cont?1:0);out++;emitted=1;cont=1;memmove(pending,pending+cut+drop,plen-cut-drop);plen-=cut+drop;}}
   if(r<e&&n&&(nextn=note_used(r+1))&&n<old){pending[plen++]=' ';if(plen>new_width){cut=0;for(k=new_width;k>0;k--)if(pending[k]==' '){cut=k;break;}drop=cut?1:0;if(!cut)cut=new_width;if(out>=NL)break;_fmemcpy(tmp+out*NW,pending,cut);ns[out]=(unsigned char)(cont?1:0);out++;emitted=1;cont=1;memmove(pending,pending+cut+drop,plen-cut-drop);plen-=cut+drop;}}
  }
  if(out<NL&&(plen||!emitted)){if(plen>new_width)plen=new_width;if(plen)_fmemcpy(tmp+out*NW,pending,plen);ns[out]=(unsigned char)(cont?1:0);out++;}
  y=e+1;
 }
 for(r=0;r<NL;r++)_fmemcpy(note+r*NW,tmp+r*NW,NW);memcpy(softwrap,ns,NL);_ffree(tmp);note_edit_width=new_width;
 if(target_para==0)target_row=0;else{hard=0;for(r=1;r<out;r++)if(!softwrap[r]){hard++;if(hard==target_para){target_row=r;break;}}}
 remain=off;r=target_row;for(;;){n=note_used(r);if(r+1<out&&softwrap[r+1]){nextn=note_used(r+1);sep=(n&&nextn)?1:0;if(remain<=n)break;remain-=n+sep;r++;continue;}break;}*pcy=r;*pcx=(int)remain;if(*pcx>n)*pcx=n;if(*pcx>=new_width)*pcx=new_width-1;if(*pcx<0)*pcx=0;note_selection_clear();return 1;
}
...
{
 int key=0,mx=0,my=0,cx=*pcx,cy=*pcy,top=*ptop,base,ch,ep,oldpos,shift,insert=*pinsert,old_close_x,old_close_y;unsigned mb=0;
 ...
 while(key!=27&&key!=256+0x85&&key!=256+0x57){
  ...
  ep=external_page_index();oldpos=cy*note_edit_width+cx;shift=note_shift_down();
  ...
  else if(key==22){note_paste(&cx,&cy,insert);if(ep>=0)external_dirty[ep]=1;else persistent_dirty=1;}
  ...
  else if(key==256+75&&cx>0){cx--;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
  else if(key==256+77&&cx<note_edit_width-1){cx++;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
  else if(key==256+72&&cy>0){cy--;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
  else if(key==256+80&&cy<NL-1){cy++;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
  else if(key==256+71){cx=0;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
  else if(key==256+79){cx=note_used(cy);if(cx>=note_edit_width)cx=note_edit_width-1;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
  else if(key==256+73){cy-=23;if(cy<0)cy=0;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
  else if(key==256+81){cy+=23;if(cy>=NL)cy=NL-1;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
  else if(key==256+82){note_selection_clear();insert=!insert;}
  else if(key==256+83){if(note_has_selection())note_delete_selection(&cx,&cy);else if(cx>=note_used(cy))note_join_next(&cx,&cy);else{base=cy*NW;memmove(note+base+cx,note+base+cx+1,note_edit_width-cx-1);note[base+note_edit_width-1]=' ';}if(ep>=0)external_dirty[ep]=1;else persistent_dirty=1;}
  else if(key==8){if(note_has_selection())note_delete_selection(&cx,&cy);else if(cx>0){cx--;base=cy*NW;memmove(note+base+cx,note+base+cx+1,note_edit_width-cx-1);note[base+note_edit_width-1]=' ';}else note_join_previous(&cx,&cy);if(ep>=0)external_dirty[ep]=1;else persistent_dirty=1;}
 ...
 if(!note_reflow(EW,&cx,&cy)){note_edit_width=EW;}top=cy>=NV?cy-NV+1:0;*pcx=cx;*pcy=cy;*ptop=top;*pinsert=insert;note_view_rows=NV;note_editor_focus=0;acc_caret_hide();acc_modal_end();acc_close_target_restore(old_close_x,old_close_y);acc_restore_text_screen();acc_mouse_display(1);
}

static void note_session_cleanup(void)
{
 int t,p;char path[ACC_PATH],name[20];for(t=0;t<MAXTABS;t++)for(p=0;p<MAXPAGES;p++){data_name(name,t,p);acc_path(path,"DATA",name);remove(path);wrap_name(name,t,p);acc_path(path,"DATA",name);remove(path);}acc_path(path,"DATA","NOTE.IDX");remove(path);
}
...
int main(int argc,char**argv)
{
 ...
 while(!done){int last;
  ...
  if(key==14){if(ctab!=external_tab&&(int)pages[ctab]<MAXPAGES){page_save();pages[ctab]++;cpage=psel=pages[ctab]-1;memset(note,' ',sizeof(note));memset(softwrap,0,sizeof(softwrap));persistent_dirty=1;meta_save();page_save();cx=cy=top=0;draw_pages(tx+2,ty+NV,pagefirst,focus==2,psel);view_draw(tx,ty,cx,cy,top);}key=0;continue;}
  ...
  oldcy=cy;oldtop=top;{int oldpos=cy*note_edit_width+cx,shift=note_shift_down(),ep=external_page_index();if(key==24||key==22||key==256+83||key==8||key==13||(key>=32&&key<=255)||(key>=513&&key<=767)){if(ep>=0)external_dirty[ep]=1;else persistent_dirty=1;}
   ...
   else if((key==22)){note_paste(&cx,&cy,insert);oldtop=-1;key=0;}
   else if(key==256+75&&cx>0){cx--;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
   else if(key==256+77&&cx<note_edit_width-1){cx++;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
   else if(key==256+72&&cy>0){cy--;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
   else if(key==256+80&&cy<NL-1){cy++;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
   else if(key==256+71){cx=0;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
   else if(key==256+79){cx=note_edit_width-1;while(cx>0&&note[cy*NW+cx]==' ')cx--;if(note[cy*NW+cx]!=' '&&cx<note_edit_width-1)cx++;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
   else if(key==256+73){cy-=NV;if(cy<0)cy=0;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
   else if(key==256+81){cy+=NV;if(cy>=NL)cy=NL-1;note_selection_move(oldpos,cy*note_edit_width+cx,shift);}
   else if(key==256+82){note_selection_clear();insert=!insert;}
   else if(key==256+83){if(note_has_selection()){note_delete_selection(&cx,&cy);oldtop=-1;}else if(cx>=note_used(cy))note_join_next(&cx,&cy);else{base=cy*NW;memmove(note+base+cx,note+base+cx+1,note_edit_width-cx-1);note[base+note_edit_width-1]=' ';}}
   else if(key==8){if(note_has_selection()){note_delete_selection(&cx,&cy);oldtop=-1;}else if(cx>0){cx--;base=cy*NW;memmove(note+base+cx,note+base+cx+1,note_edit_width-cx-1);note[base+note_edit_width-1]=' ';}else note_join_previous(&cx,&cy);}
   ...
   else if(key==27){if(!note_any_dirty())done=1;else{choice=note_confirm_close();if(choice==1){note_discard=1;done=1;}else if(choice==2&&note_save_all_dirty())done=1;else{acc_box(x,y,70,21,"Note");draw_tabs(x,y+2,tabfirst,0);view_draw(tx,ty,cx,cy,top);draw_pages(tx+2,ty+NV,pagefirst,0,psel);}}key=0;}else key=0;
  }
