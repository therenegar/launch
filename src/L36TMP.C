...
static void mouse_pointer_install(void)
{
  ...
  if(!mouse_glyph_saved){
    if(launchui_ega14())mouse_glyph_saved=2;
    else{font_plane_open(&old);font=(unsigned char far *)MAKE_FP(0xA000,127*32);for(i=0;i<32;i++)mouse_old_glyph[i]=font[i];font_plane_close(&old);mouse_glyph_saved=1;}
  }
