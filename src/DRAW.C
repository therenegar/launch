...
static void cell_draw(int x,int y,int vx,int vy,int active,int ox,int oy){int c=pixels[(oy+vy)*CW+ox+vx],attr=ACC_ATTR(c,active?15:7),l=grid_on?201:' ',r=grid_on?202:' ';acc_put(x+vx*2,y+vy,l,attr);acc_put(x+vx*2+1,y+vy,r,attr);}
...
int main(int argc,char **argv){char small[ACC_PATH],big[ACC_PATH],message[ACC_PATH*2+48],*bn;int bx,by,x,y,cx=0,cy=0,ox=0,oy=0,mx=0,my=0,colour=15,key=0,focus=0,vx,vy;unsigned mb=0;if(acc_help(argc,argv,"!DRAW","A persistent scrollable 96 by 96 colour pixel editor with cropped BMP export."))return 0;if(!acc_begin(argv[0],"Pixel Draw",0))return 1;grid_font(1);load_pixels();bx=(acc_cols-66)/2;by=(acc_rows-21)/2;x=bx+5;y=by+2;acc_box(bx,by,66,21,"Pixel Draw");draw_all(x,y,cx,cy,ox,oy,colour,focus);
 while(key!=27){acc_button(x,y+DH+4,"  Clear  ",focus==1);acc_button(x+11,y+DH+4,"  Grid  ",focus==2);acc_button(x+21,y+DH+4,"  Export  ",focus==3);acc_button(x+47,y+DH+4,"  Close  ",focus==4);acc_wait(&key,&mx,&my,&mb);
  ...
  if((mb&1)&&my==y+DH+4){if(mx<x+10)focus=1;else if(mx>=x+11&&mx<x+20)focus=2;else if(mx>=x+21&&mx<x+32)focus=3;else if(mx>=x+47)focus=4;key=13;}
  if(key==9){focus=(focus+1)%6;draw_all(x,y,cx,cy,ox,oy,colour,focus);key=0;continue;}if(focus==5){if(key==27)break;if(key==256+75&&colour%8>0)colour--;else if(key==256+77&&colour%8<7)colour++;else if(key==256+72&&colour>=8)colour-=8;else if(key==256+80&&colour<8)colour+=8;palette_draw(x,y+DH+1,colour);key=0;continue;}
  if(key==13&&focus){if(focus==1){memset(pixels,0,sizeof(pixels));cx=cy=ox=oy=0;draw_all(x,y,cx,cy,ox,oy,colour,focus);save_pixels();}else if(focus==2){grid_on=!grid_on;draw_all(x,y,cx,cy,ox,oy,colour,focus);}else if(focus==3){if(!export_bmps(small,big))acc_notice("Export","Unable to export pixel drawings.");else{bn=strrchr(big,'\\');if(!bn)bn=big;else bn++;sprintf(message,"Drawings exported to\n%s + %s",small,bn);acc_notice("Export",message);}}else key=27;if(key!=27)key=0;continue;}if(focus)focus=0;
