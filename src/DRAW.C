...
static void show_picture(void){union REGS r;int x0,y0,x1,y1,w,h,scale,sx,sy;unsigned word;crop(&x0,&y0,&x1,&y1);w=x1-x0+1;h=y1-y0+1;scale=640/w;if(350/h<scale)scale=350/h;if(scale<1)scale=1;sx=(640-w*scale)/2;sy=(350-h*scale)/2;memset(&r,0,sizeof(r));r.h.ah=0;r.h.al=0x10;int86(0x10,&r,&r);graph_picture(x0,y0,w,h,scale,sx,sy);do{word=_bios_keybrd(_KEYBRD_READ);}while((word&255)!=27);acc_restore_screen();grid_font(1);}
...
int main(int argc,char **argv){char small[ACC_PATH],big[ACC_PATH],message[ACC_PATH*2+48],*bn;int bx,by,x,y,cx=0,cy=0,ox=0,oy=0,mx=0,my=0,colour=15,key=0,focus=0,vx,vy;unsigned mb=0;if(acc_help(argc,argv,"!DRAW","A persistent scrollable 96 by 96 colour pixel editor with cropped BMP export."))return 0;if(!acc_begin(argv[0],"Pixel Draw",0))return 1;grid_font(1);load_pixels();bx=(acc_cols-66)/2;by=(acc_rows-21)/2;x=bx+3;y=by+2;acc_box(bx,by,66,21,"Pixel Draw");draw_all(x,y,cx,cy,ox,oy,colour,focus);
 while(key!=27){acc_button(x,y+DH+4,"  Clear  ",focus==1);acc_button(x+11,y+DH+4,"  Grid  ",focus==2);acc_button(x+21,y+DH+4,"  Export  ",focus==3);acc_button(x+33,y+DH+4,"  Show  ",focus==4);acc_button(x+51,y+DH+4,"  Close  ",focus==5);acc_wait(&key,&mx,&my,&mb);
  ...
  if((mb&1)&&my==y+DH+4){if(mx<x+10)focus=1;else if(mx>=x+11&&mx<x+20)focus=2;else if(mx>=x+21&&mx<x+32)focus=3;else if(mx>=x+33&&mx<x+43)focus=4;else if(mx>=x+51)focus=5;key=13;}
