...
int main(int argc,char **argv){char small[ACC_PATH],big[ACC_PATH],message[ACC_PATH*2+48],*bn,*openfile=0;int bx,by,x,y,cx=0,cy=0,ox=0,oy=0,mx=0,my=0,colour=15,key=0,focus=-1,vx,vy,i;unsigned mb=0;if(acc_help(argc,argv,"!DRAW","A persistent scrollable 96 by 96 colour pixel editor with 16-colour BMP import/export."))return 0;if(!acc_begin(argv[0],"Pixel Draw",0))return 1;load_pixels();for(i=1;i<argc;i++)if(argv[i][0]!='/'&&argv[i][0]!='-')openfile=argv[i];if(openfile&&!load_bmp(openfile))acc_notice("Open Bitmap","Bitmap must be uncompressed 16-colour BMP, maximum 96 x 96.");bx=(acc_cols-66)/2;by=(acc_rows-21)/2;x=bx+3;y=by+2;acc_box(bx,by,66,21,"Pixel Draw");draw_all(x,y,cx,cy,ox,oy,colour,focus);hover_forget();acc_set_idle_hook(draw_hover_idle);
 while(key!=27){hover_context(x,y,ox,oy,cx,cy,colour,focus);acc_button(bx+3,y+DH+4,"  Clear  ",focus==1);acc_button(bx+14,y+DH+4,"  Grid  ",focus==2);acc_button(bx+25,y+DH+4,"  Export  ",focus==3);acc_button(bx+33,y+DH+4,"  Show  ",focus==4);acc_button(bx+56,y+DH+4,"  Close  ",focus==5);acc_wait(&key,&mx,&my,&mb);
  if((mb&3)&&my>=y&&my<y+DH&&mx>=x&&mx<x+DW*2){int held=(mb&2)?2:1,buttons=held;focus=0;hover_reset();acc_mouse_display(1);do{if(my>=y&&my<y+DH&&mx>=x&&mx<x+DW*2){vx=(mx-x)/2;vy=my-y;cx=ox+vx;cy=oy+vy;pixels[cy*CW+cx]=(held==2)?0:(unsigned char)colour;cell_draw(x,y,vx,vy,0,ox,oy,colour);}acc_mouse(&mx,&my,&buttons);}while(buttons&held);key=0;continue;}
