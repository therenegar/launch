...
#define GLYPH_GOAL_R 188
#define GLYPH_MAN_L 212
#define GLYPH_MAN_R 189
...
int main(int argc,char **argv)
{
  ...
  while(key!=27){
    ...
    if(full){
      ...
      acc_button(x+11,y+17," Prev ",focus==2);
      acc_button(x+18,y+17," Next ",focus==3);
      acc_button(x+DLG_W-10,y+17," Close ",focus==4);
      full=0;dirty=0;
    }else if(dirty){
    ...
    if((mb&1)&&!(mb&ACC_MOUSE_MOVED)){
      if(my==y+17){
        ...
        else if(mx>=x+11&&mx<x+16){focus=2;change_level(-1);full=1;key=0;}
        else if(mx>=x+18&&mx<x+23){focus=3;change_level(1);full=1;key=0;}
        else if(mx>=x+DLG_W-10&&mx<x+DLG_W-4){focus=4;key=27;}
