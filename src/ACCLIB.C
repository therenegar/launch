/* Shared Launch! 3.0 accessory runtime.  Microsoft C/C++ 7.0, small model. */
#include <dos.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
ACC_APPEARANCE acc_appearance={1,11,12,14,15,10,15,3,0,7,7,0,1,1,1,1,10,0,1,1,1,0,0};
...
static void mouse_pointer_restore(void){union REGS r;if(!mouse_glyph_saved)return;mouse_glyph_write(mouse_old_glyph);memset(&r,0,sizeof(r));r.x.ax=0x000A;r.x.bx=0;r.x.cx=0xFFFF;r.x.dx=0x7700;int86(0x33,&r,&r);mouse_glyph_saved=0;}
static void mouse_pointer_install(void)
{
  ...
  if(acc_appearance.mouse_cursor){memset(&r,0,sizeof(r));r.x.ax=0x000A;r.x.bx=0;if(acc_appearance.mouse_cursor==1){r.x.cx=0xFFFF;r.x.dx=0x7700;}else{r.x.cx=0xFF00;r.x.dx=0x7708;}int86(0x33,&r,&r);return;}
...
{
  char name[ACC_PATH],line[128],*eq,*key;int value;FILE *f;unsigned char *field=0;
  ...
  while(fgets(line,sizeof(line),f)){
    ...
    if(field&&value>=0&&value<=15)*field=(unsigned char)value;
  }fclose(f);
}
...
{
  int i,len;unsigned short v;acc_fill(x,y,w,h,' ',ACC_BG);
  acc_put(x,y,218,ACC_BORDER);acc_put(x+w-1,y,191,ACC_BORDER);
  acc_put(x,y+h-1,192,ACC_BORDER);acc_put(x+w-1,y+h-1,217,ACC_BORDER);
  for(i=1;i<w-1;i++){acc_put(x+i,y,196,ACC_BORDER);acc_put(x+i,y+h-1,196,ACC_BORDER);}
  for(i=1;i<h-1;i++){acc_put(x,y+i,179,ACC_BORDER);acc_put(x+w-1,y+i,179,ACC_BORDER);}
  if(title&&*title){len=(int)strlen(title);if(len>w-10)len=w-10;acc_put(x+2,y,180,ACC_BORDER);acc_text(x+3,y,title,ACC_HEADING,len);acc_put(x+3+len,y,195,ACC_BORDER);}
  acc_put(x+w-5,y,180,ACC_BORDER);acc_put(x+w-4,y,'X',ACC_TITLE);acc_put(x+w-3,y,195,ACC_BORDER);close_x=x+w-4;close_y=y;
...
int acc_help(int argc,char **argv,const char *name,const char *description){if(argc>1&&(!stricmp(argv[1],"/?")||!stricmp(argv[1],"-?"))){printf("%s - Launch! 3.0 accessory\n\n%s\n\nThis accessory requires !.EXE in the same directory.\n",name,description);return 1;}return 0;}
